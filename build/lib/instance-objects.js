...
async function check_set_deviceData(that, jCfgDevice, jCachedDevice) {
    ...
    const fctNameId = 'io.check_set_deviceData';
    ...
    try {
        ...
        // create device node
        that.setObjectNotExistsAsync(c.dppDevices + jCfgDevice.devicename, {
            type: 'device',
            common: {
    ...
    try {
        let idState = c.dppDevices + jCfgDevice.devicename + '.' + c.idnDeviceActive;
        let idStateValue = ((jCachedDevice) && (jCachedDevice.Active) ? true : false);
        const dtCurrent = (new Date()).getTime();
...
exports.check_set_deviceData = check_set_deviceData;
async function copy_oldDeviceData(that, jCfgDeviceOld, jCfgDevice) {
    // lastActive, lastInactive übernehmen
    const fctNameId = 'io.copy_oldDeviceData';
...
exports.copy_oldDeviceData = copy_oldDeviceData;
async function delete_oldDeviceData(that, jCfgDevice) {
    // remove old objects for XXXX active, IP, lastIP, MAC, lastMAC, lastActive, lastInactive
    const fctNameId = 'io.delete_oldDeviceData';
    that.log.debug(fctNameId + ' started for jCfgDevice: ' + JSON.stringify(jCfgDevice));
...
exports.delete_oldDeviceData = delete_oldDeviceData;
